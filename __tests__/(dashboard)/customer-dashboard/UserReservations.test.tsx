...
const mockedExtendReservation = extendReservation as jest.MockedFunction<
  typeof extendReservation
>;
const mockedCheckUserCanReserve = checkUserCanReserve as jest.MockedFunction<
  typeof checkUserCanReserve
>;
...
describe("UserReservations Component", () => {
  ...
  it('shows "You don’t have any borrowed books." when the user has none', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/You don't have any borrowed books/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it("should extend a reservation successfully", async () => {
    ...
    mockCreateClient.mockReturnValueOnce(
      createMockClient([initialReservation])
    );
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/Book extended successfully!/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it("should not extend a reservation twice", async () => {
    ...
    mockCreateClient.mockReturnValueOnce(
      createMockClient([initialReservation2])
    );
    ...
    await waitFor(() => {
      ...
      expect(screen.getByRole("button", { name: /Extended/i })).toBeDisabled();
    });
